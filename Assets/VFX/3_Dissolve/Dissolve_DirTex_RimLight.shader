Shader "MineselfShader/VFX/3-Dissolve/Dissolve_DirTex_RimLight"
{
    Properties
    {
        ...
        _End("End", Float) = 1
        [Space(10)]
        [HDR]_RimCol("RimColor", COLOR) = (1,1,1,1)
        _RimLength("RimLength", Range(0, 0.1)) = 0.03
    }
    SubShader
    {
        //SubShader Tags
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //结合方向遮罩与光边的做法
                float mask1 = step(t, noiseMask + dirMask * _Scale - _RimLength);//早剔除
                float mask2 = step(t, noiseMask + dirMask * _Scale);//晚剔除
                float rimMask = mask2 - mask1;

                float3 finalRGB = lerp(mainTex, _RimCol, rimMask);
                float alpha = mask2 * mainTex.a;//注意除了需要剔除噪声图为0的部分，还要剔除主纹理本来就不要的部分
